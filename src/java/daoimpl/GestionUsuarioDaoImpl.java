    ...
    public List listarPersonal() {
        ...
        try {
            ...
            Query query = session.createSQLQuery("SELECT J.APELLIDOS||', '||J.NOMBRE AS NOMBRE,\n"
                    + "DECODE(J.USU,NULL,'SIN USUARIO',UPPER(J.USU)) AS USUARIO,\n"
                    + "DECODE(J.TELEFONO,NULL,'SIN TELEFONO',UPPER(J.TELEFONO)) AS TELEFONO,\n"
                    + "DECODE(J.ANEXO,NULL,'SIN ANEXO',UPPER(J.ANEXO)) AS ANEXO,\n"
                    + "DECODE(J.CARGO,NULL,'SIN CARGO',UPPER(J.CARGO)) AS CARGO,\n"
                    + "DECODE(J.GRADO,NULL,'SIN GRADO',UPPER(J.GRADO)) AS GRADO,\n"
                    + "DECODE(J.CORREO,NULL,'SIN CORREO',UPPER(J.CORREO)) AS CORREO,\n"
                    + "DECODE(J.DNI,NULL,'SIN DNI',UPPER(J.DNI)) AS DNI,\n"
                    + "DECODE(J.CELULAR,NULL,'SIN CELULAR',UPPER(J.CELULAR)) AS CELULAR,\n"
                    + "UPPER(P.PROFESION_NOMBRE) AS PROFESION,\n"
                    + "UPPER(D1.NOMBRE) AS OFICINA,\n"
                    + "UPPER(TP.NOMBRE_CONTRATO) AS CONTRATO\n"
                    + "\n"
