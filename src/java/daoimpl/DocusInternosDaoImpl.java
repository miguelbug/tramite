...
public class DocusInternosDaoImpl implements DocusInternosDAO {
    ...
    public String getDestinoOfi(String tram, String tramfecha) {
        String destino = "";
        session = HibernateUtil.getSessionFactory().openSession();
        System.out.println("get destino ofi");
        try {
            session.beginTransaction();
            Query query = session.createSQLQuery("SELECT D1.NOMBRE \n"
                    + "FROM OFICIOS OFI, DEPENDENCIA D1\n"
                    + "WHERE OFI.TRAM_NUM='" + tram + "'\n"
                    + "AND TO_CHAR(OFI.TRAM_FECHA,'DD/MM/YYYY')='" + tramfecha + "'\n"
                    + "AND OFI.CODIGO1=D1.CODIGO");
            destino = (String) query.uniqueResult();
            session.beginTransaction().commit();
        } catch (Exception e) {
