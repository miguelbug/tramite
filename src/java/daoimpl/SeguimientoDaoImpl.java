    ...
    public List seguimientoUser2(String oficina, String siglas) {
        ...
        try {
            ...
            Query query = session.createSQLQuery("SELECT TM.TRAM_NUM,\n"
                    ...
                    + "                    AND TM.TRAM_NUM||'-'||TM.TRAM_FECHA||'-'||MOVI_NUM NOT IN (SELECT TRAM_NUM||'-'||TRAM_FECHA||'-'||NUMERO_MOVI FROM DOCUS_INTERNOS WHERE DOCU_SIGLASINT='"+siglas+"' )\n"
    ...
    public List seguimientoUser(String oficina, String siglas) {
        ...
        try {
            ...
            Query query = session.createSQLQuery("SELECT TM.TRAM_NUM,\n"
                    ...
                    + "                    AND TDOCU.TRAM_FECHA=TM.TRAM_FECHA\n"
                    + "                    AND TDOCU.TRAM_NUM=TM.TRAM_NUM\n"
                    + "                    AND TM.TRAM_NUM||'-'||TM.TRAM_FECHA||'-'||MOVI_NUM IN (SELECT TRAM_NUM||'-'||TRAM_FECHA||'-'||NUMERO_MOVI FROM DOCUS_INTERNOS WHERE DOCU_SIGLASINT='"+siglas+"')\n"
                    + "                    ORDER BY FECHA_ENVIO DESC");
            codigos = query.list();
            session.beginTransaction().commit();
        } catch (Exception e) {
